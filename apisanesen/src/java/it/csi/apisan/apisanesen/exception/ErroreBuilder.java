...
import it.csi.apisan.apisanesen.dto.apisanesen.Dettaglio;
...
public class ErroreBuilder {
	...
	public static ErroreBuilder from(ApisanEsenStatus aas, Object... args) {
...
/*		dettaglio.setChiave(chiave);
	...
	public static Errore from(HttpStatusCodeException hsee) {
		String json = hsee.getResponseBodyAsString();
		return toObj(json, Errore.class);
	}

	private static <T>  T toObj(String json, Class<T> type) {
		T result;
		ObjectMapper mapper = new ObjectMapper();
		try {
			result = mapper.readValue(json ,type);
		} catch(Exception e) {
			String msg = String.format("Error mapping in type: %s - json: %s", type, json);
