...
@Path("/tipologie-validita")

@Produces({ "application/json" })
@io.swagger.annotations.Api(description = "the tipologie-validita API")

public interface TipologieValiditaApi  {
   
    @GET
    
    
    @Produces({ "application/json" })
    @io.swagger.annotations.ApiOperation(value = "Lista delle tipologie di validità", notes = "Restituiscee il tipo del consenso, possibili valori   * A - AZIENDALE   * R - REGIONALE ", response = TipoConsenso.class, responseContainer = "List", tags={  })
    @io.swagger.annotations.ApiResponses(value = { 
        @io.swagger.annotations.ApiResponse(code = 200, message = "La richiesta è andata a buon fine e restituisce l'array contenente le tipologie di validità", response = TipoConsenso.class, responseContainer = "List"),
