...
import it.csi.apisan.apisanprof.business.apisanprof.impl.base.service.ServizioAttivoGetService;
...
public abstract class RESTBaseService {
	...
	public Response executeService() {
		...
		try {
			...
			if(this.securityContext.isUserInRole(Constants.RUOLO_MONITORAGGIO) && !this.getClass().getSimpleName().equals(ServizioAttivoGetService.class.getSimpleName())) {
				 throw  ApisanProfStatus.UTENTE_NON_AUTORIZZATO.getRestException("Utente non autorizzato a compiere l'operazione"); 
			}
			res = execute();
		} catch (Exception e) {
			handleException(e);
	...
	protected void handleException(Exception e) {
		...
		log.error(METHOD_NAME, msg.replaceAll("%", "%%"), e);
		Errore err = new ErroreBuilder(500, msg).getErrore();
		res = Response.status(err.getStatus()).type(MediaType.APPLICATION_JSON_TYPE).entity(err).build();
//		res = Response.status(500).type(MediaType.TEXT_HTML).entity(msg).build();
	...
	protected void checkNotNull(Object obj, String message) {
		checkNotNull(obj, new RestErroreException(Status.BAD_REQUEST,  message, message));
	}
	...
	protected void checkNotBlank(String str, String message) {
		checkNotBlank(str, new RestErroreException(Status.BAD_REQUEST,  message, message));
	}
	...
	protected void checkCondition(boolean isOk, String message) {
		checkCondition(isOk, new RestErroreException(Status.BAD_REQUEST,  message, message));
	}
}
