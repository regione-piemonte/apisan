...
public interface AuraApi {
	...
	@GET
    @Path("/{cit_id}/esenzioni-aura/{cod_esenzione}/{cod_malattia}")
    
    
    @io.swagger.annotations.ApiOperation(value = "Dettaglio esenzione da aura", notes = "Permette di ottenere la singola esenzione da Aura associata al cittadino e identificata da cod_esenzione e dal filtro obbligatorio su data_emissione e data_scadenza ", response = ModelEsenzioneAura.class, tags={ "Dinamiche", })
    @io.swagger.annotations.ApiResponses(value = { 
        @io.swagger.annotations.ApiResponse(code = 200, message = "Operazione eseguita con successo ", response = ModelEsenzioneAura.class),
    ...
    @io.swagger.annotations.ApiResponses(value = { 
        ...
        @io.swagger.annotations.ApiResponse(code = 500, message = "Si è verificato un'errore interno del server. (es. null pointer exception, connessione al DB saltata ecc)", response = Errore.class) })
    public Response cittadiniCitIdEsenzioniAuraCodEsenzioneRevocaPut(@ApiParam(value = "Contiene il codice fiscale dell'utente loggato ottenuto tramite shibboleth" ,required=true)@HeaderParam("Shib-Identita-CodiceFiscale") String shibIdentitaCodiceFiscale,@ApiParam(value = "Contiene un id univoco (`uuid v4`) della chiamata HTTP che viene generato dal front-end, viene propagato agli strati successivi e viene restituito al front-end assieme alla response, permettendo così di tracciare l'intero ciclo di vita della richiesta" ,required=true)@HeaderParam("X-Request-Id") String xRequestId,@ApiParam(value = "Contiene l'elenco degli ip che hanno gestito la richiesta. Utile per i log e l'audit. Il formato della stringa è  ``` <ip-front-end>, <ip-primo-server>, ..., <ip-ultimo-server> ``` Es. `10.10.10.10, 10.38.45.68, 10.128.69.244` " ,required=true)@HeaderParam("X-Forwarded-For") String xForwardedFor,@ApiParam(value = "Contiene una stringa identificativa del servizio chiamante (es.SANSOL)" ,required=true)@HeaderParam("X-Codice-Servizio") String xCodiceServizio, @PathParam("cit_id") String citId, @PathParam("cod_esenzione") String codEsenzione, @PathParam("cod_malattia") String codMalattia, @NotNull @QueryParam("filter") String filter,@Context SecurityContext securityContext, @Context HttpHeaders httpHeaders, @Context HttpServletRequest request, @Context HttpServletResponse response );
    @GET
    @Path("/{cit_id}/esenzioni-aura")
    
    
    @io.swagger.annotations.ApiOperation(value = "Lista esenzioni da aura", notes = "Permette di ottenere la lista delle esenzioni da Aura associate al cittadino ed eventualmente di filtrarle tramite il parametro `filter` nella query string ", response = ModelEsenzioneAura.class, responseContainer = "List", tags={ "Dinamiche", })
    @io.swagger.annotations.ApiResponses(value = { 
        @io.swagger.annotations.ApiResponse(code = 200, message = "Operazione eseguita con successo ", response = ModelEsenzioneAura.class, responseContainer = "List"),
