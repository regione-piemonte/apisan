...
private NamedParameterJdbcTemplate namedJdbcTemplate;
	...
	private final static String SQL_INSERT_T_LOG_CHIAMATA = 
			"insert into " + ConstantsEsen.TABELLA_LOG + " ( x_request_id, x_codice_servizio, ip_chiamante, cf_richiedente, componente, esito_chiamata,"
			+ "request_payload, response_payload, request_date, response_date, metodo, request_uri ) "
			+ "values ( :x_request_id, :x_codice_servizio, :ip_chiamante, :cf_richiedente, :componente, :esito_chiamata,"
	...
	private final static String SQL_UPDATE_T_LOG_CHIAMATA = 
			"update " + ConstantsEsen.TABELLA_LOG + " set response_payload = pgp_sym_encrypt_bytea(:response_payload, :encryption_key ),  response_date = now(), esito_chiamata = :esito_chiamata, millis_response = :millis_response"
			+ " where esen_chiamate_id = :esen_chiamate_id";
	...
	public long insertTLogChiamata(ApisanTrace trace) throws DataAccessException{
		...
		namedJdbcTemplate.update(SQL_INSERT_T_LOG_CHIAMATA, params, keyHolder, new String[]{"esen_chiamate_id"} ); 
	...
	public int updateTLogChiamata(ApisanTrace trace, Long id) {
		...
		params.addValue("esen_chiamate_id", id, Types.BIGINT);  
