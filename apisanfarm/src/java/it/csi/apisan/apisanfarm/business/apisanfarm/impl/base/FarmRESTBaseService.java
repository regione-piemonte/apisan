...
import it.csi.apisan.apisanfarm.util.ApisanFarmStatus;


public abstract class FarmRESTBaseService extends RESTBaseService {
	...
	public FarmRESTBaseService(SecurityContext securityContext, HttpHeaders httpHeaders, HttpServletRequest httpRequest) {
...
/*	public ConsRESTBaseService(String shibIdentitaCodiceFiscale, String xRequestID, String xCodiceServizio,
	...
	public Response executeService() {
		if(StringUtils.isBlank(xRequestID)) {
			return ErroreBuilder.from(ApisanFarmStatus.PARAMETRO_NON_VALIDO, "X-Request-ID").exception().getResponse();
		}
	...
	protected void checkUtenteAutorizzato(String cf) {
		checkCondition(isUtenteAutorizzato(cf), ErroreBuilder.from(ApisanFarmStatus.UTENTE_NON_AUTORIZZATO).dettaglio("CODICE_FISCALE", cf).exception());
	}
	...
	protected void checkCodiceFiscaleValido(String cf) {
		checkCondition(isCodiceFiscaleValido(cf), ErroreBuilder.from(ApisanFarmStatus.CODICE_FISCALE_NON_VALIDO, cf).dettaglio("CODICE_FISCALE", cf).exception());
	}
