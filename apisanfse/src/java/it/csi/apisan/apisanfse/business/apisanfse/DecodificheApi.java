...
public interface DecodificheApi  {
    ...
    @Produces({ "application/json" })
    @io.swagger.annotations.ApiOperation(value = "Get categorie", notes = "Ritorna le categorie presenti con i tipi di documento associate", response = ModelCategoria.class, responseContainer = "List", tags={ "Documenti", })
    @io.swagger.annotations.ApiResponses(value = { 
        @io.swagger.annotations.ApiResponse(code = 200, message = "OK", response = ModelCategoria.class, responseContainer = "List"),
    ...
    public Response decodificheDocumentiCategorieGet(@ApiParam(value = "Contiene un id univoco (`uuid v4`) della chiamata HTTP che viene generato dal front-end, viene propagato agli strati successivi e viene restituito al front-end assieme alla response, permettendo così di tracciare l'intero ciclo di vita della richiesta" ,required=true)@HeaderParam("X-Request-Id") String xRequestId,@ApiParam(value = "Contiene l'elenco degli ip che hanno gestito la richiesta. Utile per i log e l'audit. Il formato della stringa è ``` <ip-front-end>, <ip-primo-server>, ..., <ip-ultimo-server> ``` Es. `10.10.10.10, 10.38.45.68, 10.128.69.244` " ,required=true)@HeaderParam("X-Forwarded-For") String xForwardedFor,@ApiParam(value = "Contiene una stringa identificativa del servizio chiamante (es.SANSOL)" ,required=true)@HeaderParam("X-Codice-Servizio") String xCodiceServizio,@ApiParam(value = "Contiene il codice fiscale dell'utente loggato ottenuto tramite shibboleth" ,required=true)@HeaderParam("Shib-Identita-CodiceFiscale") String shibIdentitaCodiceFiscale, @NotNull @QueryParam("codice_app_verticale") String codiceAppVerticale,@Context SecurityContext securityContext, @Context HttpHeaders httpHeaders );
    @GET
    @Path("/documenti/tipi-episodio")
    
    @Produces({ "application/json" })
    @io.swagger.annotations.ApiOperation(value = "get tipi episodio", notes = "Restituisce i tipi di episodio ", response = ModelTipoEpisodio.class, responseContainer = "List", tags={ "Documenti", })
    @io.swagger.annotations.ApiResponses(value = { 
        @io.swagger.annotations.ApiResponse(code = 200, message = "Operazione eseguita con successo ", response = ModelTipoEpisodio.class, responseContainer = "List"),
