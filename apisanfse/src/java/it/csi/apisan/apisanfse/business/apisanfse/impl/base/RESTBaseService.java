...
import it.csi.apisan.apisanfse.exception.RESTException;
...
public abstract class RESTBaseService {
	...
	public Response executeService() {
		...
		try {
			if(this.securityContext.isUserInRole(ConstantsFse.RUOLO_MONITORAGGIO) && !this.getClass().getSimpleName().equals(ServizioAttivoGetService.class.getSimpleName())) {
			...
			/*try {
				...
				//creo unna apisantrace e inserisco
				ApisanTrace trace = new ApisanTrace();
				
				trace.setCfRichiedente(this.shibIdentitaCodiceFiscale);
				trace.setComponente(Constants.COMPONENT_NAME);
				trace.setIpChiamante(this.xForwardedFor);
				trace.setxRequestId(this.xRequestID);
				trace.setEsitoChiamata(res.getStatus());
				traceOperation.traceAll(trace);
			}catch(Exception e) {
				log.error(METHOD_NAME, "ERRORE NELL'INSERIMENTO DELLA TRACCIATURA", this.getClass().getSimpleName());
			}*/
