...
import it.csi.apisan.apisanfse.util.StatoDelega;
...
public abstract class FseDelegheRESTBaseService extends FseRESTBaseService {
	...
	public FseDelegheRESTBaseService(SecurityContext securityContext, HttpHeaders httpHeaders) {
		super(securityContext, httpHeaders);
	}
	
	
	public FseDelegheRESTBaseService(String shibIdentitaCodiceFiscale, String xRequestID, String xCodiceServizio,
	...
	protected void checkUtenteAutorizzatoOrDelegato(String cf, String codiceServizio) {
		checkNotNull(shibIdentitaCodiceFiscale, ErroreBuilder.from(ApisanFseStatus.UTENTE_NON_AUTORIZZATO).exception());
		if(!isUtenteAutorizzato(cf)) {
			checkCondition(isUtenteDelegato( cf, codiceServizio), ErroreBuilder.from(ApisanFseStatus.UTENTE_NON_AUTORIZZATO).exception());
		}
	}	
	...
	protected void verificaUtenteAutorizzatoODelegato(String cf, String codiceServizio, Delega delega) {
		checkNotNull(shibIdentitaCodiceFiscale, ErroreBuilder.from(ApisanFseStatus.UTENTE_NON_AUTORIZZATO).exception());
		if(!isUtenteAutorizzato(cf)) {
			checkCondition(isUtenteDelegatoGradoTipo( cf, codiceServizio, delega), ErroreBuilder.from(ApisanFseStatus.UTENTE_NON_AUTORIZZATO).exception());
		}
	}
