...
import it.csi.apisan.apisanpag.util.trace.TraceRestInterceptor;
...
public class RestCaller {
	...
	private List<HttpStatus> httpStatusNotMapping;
	...
	public RestCaller httpStatusNotMapping(HttpStatus statusCode) {
		if(httpStatusNotMapping == null) {
			httpStatusNotMapping = new ArrayList<HttpStatus>();
		}
		httpStatusNotMapping.add(statusCode);
		return this;
	}
	...
	public <T> T call(TypeReference<T> type) {
		...
		log.info(METHOD_NAME, "url: %s", url);
		...
		try {
			if(encodingUTF8) {
				log.info(METHOD_NAME, "encoding uf8");
			...
			URI uri = new URI(url);
			re = restTemplate.exchange(url, httpMethod, httpEntity, String.class);
		...
		if(httpStatusNotMapping != null && httpStatusNotMapping.contains(re.getStatusCode())) {
			return null;
		}
