...
******************************************************************************/
package it.csi.apisan.apisanpag.business.integration.dao;
...
public class ApisanTLogDao extends JdbcDaoSupport {
	...
	@Value("${apisanpagTabellaLog}")
	private String apisanpagTabellaLog;
	@Value("${apisanpagIdTabellaLog}")
	private String apisanpagIdTabellaLog;
	
	private  String SQL_INSERT_T_LOG_CHIAMATA =  
			"insert into @apisanpagTabellaLog@ ( x_request_id, cf_chiamante, x_codice_servizio, ip_chiamante, cf_richiedente, componente, esito_chiamata,"
	...
	private  String SQL_UPDATE_T_LOG_CHIAMATA =  
			"update @apisanpagTabellaLog@ set response_payload = pgp_sym_encrypt_bytea(:response_payload, :encryption_key ),  response_date = now(), esito_chiamata = :esito_chiamata, millis_response = :millis_response"
			+ " where @apisanpagIdTabellaLog@ = :pag_t_log_chiamate_id";
	...
	public long insertTLogChiamata(ApisanTrace trace) throws DataAccessException{
		...
		namedJdbcTemplate.update(SQL_INSERT_T_LOG_CHIAMATA, params, keyHolder, new String[]{apisanpagIdTabellaLog} ); 
	...
	public int updateTLogChiamata(ApisanTrace trace, Long id) {
		...
		params.addValue("pag_t_log_chiamate_id", id, Types.BIGINT);  
