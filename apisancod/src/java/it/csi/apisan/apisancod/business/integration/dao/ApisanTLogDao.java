...
******************************************************************************/
package it.csi.apisan.apisancod.business.integration.dao;
...
private NamedParameterJdbcTemplate namedJdbcTemplate;
	...
	private final static String SQL_INSERT_T_LOG_CHIAMATA = 
			"insert into " + ConstantsCod.TABELLA_LOG + " ( x_request_id, cf_chiamante, x_codice_servizio, ip_chiamante, cf_richiedente, componente, esito_chiamata,"
	...
	private final static String SQL_UPDATE_T_LOG_CHIAMATA = 
			"update " + ConstantsCod.TABELLA_LOG + " set response_payload = pgp_sym_encrypt_bytea(:response_payload, :encryption_key ),  response_date = now(), esito_chiamata = :esito_chiamata, millis_response = :millis_response"
			+ " where cod_chiamate_id = :cod_chiamate_id";
	...
	public long insertTLogChiamata(ApisanTrace trace) throws DataAccessException{
		...
		namedJdbcTemplate.update(SQL_INSERT_T_LOG_CHIAMATA, params, keyHolder, new String[]{"cod_chiamate_id"} ); 
	...
	public int updateTLogChiamata(ApisanTrace trace, Long id) {
		...
		params.addValue("cod_chiamate_id", id, Types.BIGINT);  
