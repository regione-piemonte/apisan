...
import it.csi.apisan.apisanceliachia.business.apisanceliachia.impl.base.service.ServizioAttivoGetService;
import it.csi.apisan.apisanceliachia.exception.ErroreBuilder;
import it.csi.apisan.apisanceliachia.exception.RESTException;
...
public abstract class RESTBaseService {
	...
	public Response executeService() {
		...
		try {
			if(this.securityContext.isUserInRole(ConstantsCeliachia.RUOLO_MONITORAGGIO) && !this.getClass().getSimpleName().equals(ServizioAttivoGetService.class.getSimpleName())) {
				throw ErroreBuilder.from(401, "UNAUTHORIZED").exception(); 
			}
			res = execute();
		} catch (Exception e) {
			handleException(e);
