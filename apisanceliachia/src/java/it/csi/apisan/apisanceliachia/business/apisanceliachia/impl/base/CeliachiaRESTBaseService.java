...
public abstract class CeliachiaRESTBaseService extends RESTBaseService {
	...
	public CeliachiaRESTBaseService(SecurityContext securityContext, HttpHeaders httpHeaders, HttpServletRequest httpRequest) {
...
/*	public ConsRESTBaseService(String shibIdentitaCodiceFiscale, String xRequestID, String xCodiceServizio,
	...
	public Response executeService() {
		if(StringUtils.isBlank(xRequestID)) {
			return ErroreBuilder.from(ApisanCeliachiaStatus.PARAMETRO_NON_VALIDO, "X-Request-ID").exception().getResponse();
		}
	...
	protected void checkUtenteAutorizzato(String cf) {
		checkCondition(isUtenteAutorizzato(cf), ErroreBuilder.from(ApisanCeliachiaStatus.UTENTE_NON_AUTORIZZATO).dettaglio("CODICE_FISCALE", cf).exception());
	}
	...
	protected void checkCodiceFiscaleValido(String cf) {
		checkCondition(isCodiceFiscaleValido(cf), ErroreBuilder.from(ApisanCeliachiaStatus.CODICE_FISCALE_NON_VALIDO, cf).dettaglio("CODICE_FISCALE", cf).exception());
	}
