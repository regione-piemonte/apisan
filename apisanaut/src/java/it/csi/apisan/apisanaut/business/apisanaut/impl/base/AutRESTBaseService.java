...
public abstract class AutRESTBaseService extends RESTBaseService {

	protected String shibIdentitaCodiceFiscale;
	protected String xRequestID;
	protected String xCodiceServizio;

	public AutRESTBaseService(SecurityContext securityContext, HttpHeaders httpHeaders) {
	...
	public AutRESTBaseService(String shibIdentitaCodiceFiscale, String xRequestID, String xCodiceServizio,
	...
	public Response executeService() {
		if(StringUtils.isBlank(xRequestID)) {
			return ErroreBuilder.from(ApisanAutStatus.PARAMETRO_NON_VALIDO, "X-Request-ID").exception().getResponse();
		}
	...
	protected void checkUtenteAutorizzato(String cf) {
		checkCondition(isUtenteAutorizzato(cf), ErroreBuilder.from(ApisanAutStatus.UTENTE_NON_AUTORIZZATO).dettaglio("CODICE_FISCALE", cf).exception());
	}
	...
	protected void checkCodiceFiscaleValido(String cf) {
		checkCondition(isCodiceFiscaleValido(cf), ErroreBuilder.from(ApisanAutStatus.CODICE_FISCALE_NON_VALIDO, cf).dettaglio("CODICE_FISCALE", cf).exception());
	}
