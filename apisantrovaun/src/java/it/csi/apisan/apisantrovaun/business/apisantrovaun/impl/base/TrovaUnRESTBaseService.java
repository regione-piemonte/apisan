...
public abstract class TrovaUnRESTBaseService extends RESTBaseService {
	...
	public TrovaUnRESTBaseService(SecurityContext securityContext, HttpHeaders httpHeaders) {
	...
	protected void checkUtenteAutorizzato(String cf) {
		checkCondition(isUtenteAutorizzato(cf), ErroreBuilder.from(ApisanTrovaUnStatus.UTENTE_NON_AUTORIZZATO).dettaglio("CODICE_FISCALE", cf).exception());
	}
	...
	protected void checkCodiceFiscaleValido(String cf) {
		checkCondition(isCodiceFiscaleValido(cf), ErroreBuilder.from(ApisanTrovaUnStatus.CODICE_FISCALE_NON_VALIDO, cf).dettaglio("CODICE_FISCALE", cf).exception());
	}
