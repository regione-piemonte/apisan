...
@io.swagger.annotations.Api(description = "the farmacie API")

public interface FarmacieApi  {
   
    @GET
    
    
    @Produces({ "application/json" })
    @io.swagger.annotations.ApiOperation(value = "Lista farmacie", notes = "Lista delle farmacie.  La lista è paginata tramite i query params opzionali `limit` ed `offset`.  Permette di filtrare la lista  per nome, vicinanza geografica e tutte le combinazioni.  In particolare:   * Se viene passato solo il query param `s` allora viene eseguita la ricerca solo per full-text   * Se vengono passati solo i query params `latitudine`, `longitudine` e `distanza`, allora viene eseguita la ricerca per vicinanza geografica    E' possibile poi combinare i query params per raffinare maggiormente la ricerca. ", response = ModelFarmacia.class, responseContainer = "List", tags={  })
    @io.swagger.annotations.ApiResponses(value = { 
        @io.swagger.annotations.ApiResponse(code = 200, message = "Operazione eseguita con successo ", response = ModelFarmacia.class, responseContainer = "List"),
