...
@io.swagger.annotations.Api(description = "the ospedali API")

public interface OspedaliApi  {
   
    @GET
    
    
    @Produces({ "application/json" })
    @io.swagger.annotations.ApiOperation(value = "Lista ospedali", notes = "Lista degli ospedali.  La lista é paginata tramite i query params opzionali `limit` ed `offset`.  Permette di filtrare la lista  per nome, vicinanza geografica, tipologia e tutte le combinazioni.      In particolare:   * Se viene passato solo il query param `s` allora viene eseguita la ricerca solo per full-text   * Se vengono passati solo i query params `latitudine`, `longitudine` e `distanza`, allora viene eseguita la ricerca per vicinanza geografica   * Se viene passato solo il query params `tipologia` allora viene eseguita la ricerca solo per determinate tipologie.        E' possibile poi combinare i query params per raffinare maggiormente la ricerca. ", response = ModelOspedale.class, responseContainer = "List", tags={  })
    @io.swagger.annotations.ApiResponses(value = { 
        @io.swagger.annotations.ApiResponse(code = 200, message = "Operazione eseguita con successo. ", response = ModelOspedale.class, responseContainer = "List"),
