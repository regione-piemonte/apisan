...
import it.csi.apisan.apisanscerev.util.ScerevConstants.HeaderParam;
...
public class RestCaller {
	...
	public <T> T call(TypeReference<T> type) {
		...
		//stopWatch.start();
		ResponseEntity<Loccsi[]> re = null;
		try {
			if(encodingUTF8) {
				restTemplate.getMessageConverters().add(0, new StringHttpMessageConverter(Charset.forName("UTF-8")));
			}
			URI uri = new URI(url);
			re = restTemplate.exchange(uri, httpMethod, httpEntity, Loccsi[].class);
		} catch (URISyntaxException e1) {
			// TODO Auto-generated catch block
			throw new RestClientException(e1.getMessage(), e1);
		}
		
		Loccsi[] json = re.getBody();	
		if (json == null) {
			return null;
		}
		String tr = json.toString(); 
		...
		try {
			result = mapper.readValue(new String(json.toString()) ,type);
		} catch(Exception e) {
