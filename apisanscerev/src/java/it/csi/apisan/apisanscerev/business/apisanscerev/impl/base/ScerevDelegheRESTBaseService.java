...
import it.csi.apisan.apisanscerev.util.StatoDelega;
...
public abstract class ScerevDelegheRESTBaseService extends ScerevRESTBaseService {

	@Autowired
	protected DelegheCittadiniService delegheCittadiniService;
	
	@Value("${serviceSCEREVName}")
	private String servicescerevName;
	
	
	public ScerevDelegheRESTBaseService(SecurityContext securityContext, HttpHeaders httpHeaders) {
		super(securityContext, httpHeaders);
		
	}
	
	protected void checkUtenteAutorizzatoOrDelegato(String cf) {
		checkUtenteAutorizzatoOrDelegato( cf, servicescerevName); 
	}
	...
	protected void checkUtenteAutorizzatoOrDelegato(String cf, String codiceServizio) {
		checkNotNull(shibIdentitaCodiceFiscale, ErroreBuilder.from(ApisanScerevStatus.UTENTE_NON_AUTORIZZATO).exception());
		if(!isUtenteAutorizzato(cf)) {
			checkCondition(isUtenteDelegato( cf, codiceServizio), ErroreBuilder.from(ApisanScerevStatus.UTENTE_NON_AUTORIZZATO).exception());
		}
	}	
