...
******************************************************************************/
package it.csi.apisan.apisanscerev.business.integration.dao;
...
private NamedParameterJdbcTemplate namedJdbcTemplate;
	...
	private final static String SQL_INSERT_T_LOG_CHIAMATA = 
			"insert into " + ScerevConstants.TABELLA_LOG + " ( x_request_id, cf_chiamante, x_codice_servizio, ip_chiamante, cf_richiedente, componente, esito_chiamata,"
	...
	private final static String SQL_UPDATE_T_LOG_CHIAMATA = 
			"update " + ScerevConstants.TABELLA_LOG + " set response_payload = pgp_sym_encrypt_bytea(:response_payload, :encryption_key ),  response_date = now(), esito_chiamata = :esito_chiamata, millis_response = :millis_response"
			+ " where scerev_t_log_chiamate_id = :scerev_t_log_chiamate_id";
	...
	public long insertTLogChiamata(ApisanTrace trace) throws DataAccessException{
		...
		namedJdbcTemplate.update(SQL_INSERT_T_LOG_CHIAMATA, params, keyHolder, new String[]{"scerev_t_log_chiamate_id"} ); 
	...
	public int updateTLogChiamata(ApisanTrace trace, Long id) {
		...
		params.addValue("scerev_t_log_chiamate_id", id, Types.BIGINT);  
