...
public final class DateTimeUtil implements Serializable {
  ...
  /**
   * Instantiates {@link DateTimeUtil} from a {@link Date} and {@link TimeZone}.
   *
   * @param date date and time
   * @param zone time zone; if {@code null}, it is interpreted as {@code TimeZone.getDefault()}.
   */
  public DateTimeUtil(Date date, TimeZone zone) {
    this(false, date.getTime(), zone == null ? null : zone.getOffset(date.getTime()) / 60000);
  }

  /**
   * Instantiates {@link DateTimeUtil} from the number of milliseconds since the Unix epoch.
   *
   * <p>
   ...
   */
  public DateTimeUtil(long value) {
    this(false, value, null);
  }

  /**
   * Instantiates {@link DateTimeUtil} from a {@link Date}.
   *
   * <p>
   * The time zone is interpreted as {@code TimeZone.getDefault()}, which may vary with
   * implementation.
   ...
   */
  public DateTimeUtil(Date value) {
    this(value.getTime());
  }

  /**
   * Instantiates {@link DateTimeUtil} from the number of milliseconds since the Unix epoch, and a shift
   * from UTC in minutes.
   *
   * @param value number of milliseconds since the Unix epoch (January 1, 1970, 00:00:00 GMT)
   * @param tzShift time zone, represented by the number of minutes off of UTC.
   */
  public DateTimeUtil(long value, int tzShift) {
    this(false, value, tzShift);
  }

  /**
   * Instantiates {@link DateTimeUtil}, which may represent a date-only value, from the number of
   ...
   */
  public DateTimeUtil(boolean dateOnly, long value, Integer tzShift) {
  ...
  /**
   * Instantiates {@link DateTimeUtil} from an <a href='http://tools.ietf.org/html/rfc3339'>RFC 3339</a>
   * date/time value.
   *
   * <p>
   ...
   * @since 1.11
   */
  public DateTimeUtil(String value) {
    ...
    // "return new DateTimeUtil(str);".
    DateTimeUtil DateTimeUtil = parseRfc3339(value);
    this.dateOnly = DateTimeUtil.dateOnly;
    this.value = DateTimeUtil.value;
    this.tzShift = DateTimeUtil.tzShift;
  }
  ...
  public String toStringRfc3339() {
    ...
    Calendar DateTimeUtil = new GregorianCalendar(GMT);
    long localTime = value + (tzShift * 60000L);
    DateTimeUtil.setTimeInMillis(localTime);
    // date
    appendInt(sb, DateTimeUtil.get(Calendar.YEAR), 4);
    sb.append('-');
    appendInt(sb, DateTimeUtil.get(Calendar.MONTH) + 1, 2);
    sb.append('-');
    appendInt(sb, DateTimeUtil.get(Calendar.DAY_OF_MONTH), 2);
    if (!dateOnly) {
      // time
      sb.append('T');
      appendInt(sb, DateTimeUtil.get(Calendar.HOUR_OF_DAY), 2);
      sb.append(':');
      appendInt(sb, DateTimeUtil.get(Calendar.MINUTE), 2);
      sb.append(':');
      appendInt(sb, DateTimeUtil.get(Calendar.SECOND), 2);

      if (DateTimeUtil.isSet(Calendar.MILLISECOND)) {
        sb.append('.');
        appendInt(sb, DateTimeUtil.get(Calendar.MILLISECOND), 3);
      }
      // time zone
      if (tzShift == 0) {
        sb.append('Z');
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof DateTimeUtil)) {
      return false;
    }
    DateTimeUtil other = (DateTimeUtil) o;
   ...
   * @throws NumberFormatException if {@code str} doesn't matcTh the RFC3339 standard format; an
   *         exception is thrown if {@code str} doesn't match {@code RFC3339_REGEX} or if it
   *         contains a time zone shift but no time.
   */
  public static DateTimeUtil parseRfc3339(String str) throws NumberFormatException {
    ...
    if (isTimeGiven) {
      ...
      if (matcher.group(8) != null) { // contains .milliseconds?
        ...
        milliseconds = (int) (milliseconds / Math.pow(10, fractionDigits));
      }
    }
    Calendar DateTimeUtil = new GregorianCalendar(GMT);
    DateTimeUtil.set(year, month, day, hourOfDay, minute, second);
    DateTimeUtil.set(Calendar.MILLISECOND, milliseconds);
    long value = DateTimeUtil.getTimeInMillis();
    ...
    return new DateTimeUtil(!isTimeGiven, value, tzShiftInteger);
  }
