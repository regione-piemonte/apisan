...
******************************************************************************/
package it.csi.apisan.apisanffe.util.interceptor;
...
import it.csi.apisan.apisanffe.util.LogUtil;
import it.csi.apisan.apisanffe.util.SpringApplicationContextProvider;
import it.csi.apisan.apisanffe.util.trace.ApisanTraceOperationDB;
...
public class TraceRequestInterceptor implements PreProcessInterceptor, PostProcessInterceptor {
	...
	public ServerResponse preProcess(HttpRequest request, ResourceMethod arg1) throws Failure, WebApplicationException {
		...
		try {
			...
			tr.setComponente(ConstantsFfe.COMPONENT_NAME);
